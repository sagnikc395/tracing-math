...
errors occur $0.59$ steps late on average. A post-hoc MiniLM baseline using only the visible prefix
reaches AUROC $0.753$. A post-hoc matched transition probe reaches AUROC $0.769$ on 380 held-out
pairs, but placebo reuse and covariate imbalance make this result exploratory. The revised
behavioral readout has AUROC $0.342$, so it fails the intervention validity gate. The evidence
supports incremental predictive information in this case study. It does not establish generalization,
calibration, self-monitoring, localization, or causal use.
\end{abstract}
...
As a separate post-hoc semantic control, we encode the problem and prefix through the current
boundary with mean-pooled \texttt{sentence-transformers/all-MiniLM-L6-v2} embeddings, then fit the
same trace-equal linear-head, validation-$C$, and validation-threshold procedure. This control has
no future steps, final-answer field, or target-model hidden state. Its result record does not pin an
encoder revision or preserve paired bootstrap draws, so we report point estimates only.
...
The post-hoc contextual prefix baseline selected $C=0.01$ with validation AUROC $0.758$. On the
same 669 held-out traces, it reaches AUROC $0.753$, average precision $0.673$, log loss $0.596$,
and Process F1 $0.268$. These point estimates are below the hidden probe, but the saved result has
no paired interval and the encoder revision is not pinned. This single baseline does not rule out
other visible-text representations.
...
Contextual prefix (MiniLM) & .753 & .268 & -- & -- \\
...
target labels all later boundaries after an annotated error. The MiniLM prefix baseline is one
unpaired post-hoc comparison with an unpinned encoder revision; it does not cover every textual or
generator shortcut. The final-answer field is unavailable online. The source transfer analysis
measures ranking, while threshold-dependent metrics vary more strongly. Probability calibration was
not tested as a separate research question.
...
prefix text and structural metadata on the inspected split. A contextual visible-prefix baseline is
also below the hidden probe by point estimate, but it is one unpaired post-hoc run. The gain is not a
complete verifier: localization remains weak, correct-trace rejection is limited, threshold behavior
varies across sources and lengths, and the native behavioral readout fails its AUROC gate. The
appropriate claim is therefore incremental predictive information in a bounded case study, not
localization, self-monitoring, generalization, calibration, or causal use.
