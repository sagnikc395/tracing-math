"""Resumable orchestration for extraction, fitting, and artifact I/O."""
...
from tracing_math.probes import ProbeResults, binary_metrics, fit_layer_probes


def download_data(config: ProjectConfig, *, force: bool = False) -> Path:
...
def extract_activation_shards(config: ProjectConfig) -> dict[str, int]:
...
def fit_and_save_probes(config: ProjectConfig) -> ProbeResults:
...
def run_and_save_interventions(config: ProjectConfig) -> pd.DataFrame:
    traces = load_traces(config.data.output_path)
...
def plot_artifacts(config: ProjectConfig) -> list[Path]:
...
def _check_extraction_identity(config: ProjectConfig, output_dir: Path) -> None:
