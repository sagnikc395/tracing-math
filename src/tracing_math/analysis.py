"""CPU analyses over frozen prediction and intervention artifacts."""
...
from tracing_math.data import ProcessTrace, assign_partitions, load_traces
...
def run_analysis(config: ProjectConfig) -> dict[str, Any]:
    """Run the complete analysis without loading a language model or activation shard."""
    LOGGER.info("Loading frozen prediction artifacts")
    predictions = _load_predictions(config.extraction.output_dir)
    threshold = _load_threshold(config.extraction.output_dir, predictions)
    traces = _trace_series(predictions)
    source_traces = load_traces(config.data.output_path)
    output = config.artifacts.analysis_dir
    output.mkdir(parents=True, exist_ok=True)

    LOGGER.info("Running temporal randomization (%d draws)", config.analysis.permutation_samples)
    permutation_summary, permutation_draws = temporal_randomization_test(
        traces,
        threshold=threshold,
        samples=config.analysis.permutation_samples,
        seed=config.seed,
    ...
    LOGGER.info(
        "Running error-aligned event study (%d draws)",
        config.analysis.exploratory_bootstrap_samples,
    )
    trajectory = event_study(
        traces,
        samples=config.analysis.exploratory_bootstrap_samples,
        seed=config.seed,
        confidence_level=config.analysis.confidence_level,
    )
    ...
    LOGGER.info(
        "Running matched-placebo analysis (%d draws)",
        config.analysis.exploratory_bootstrap_samples,
    )
    ...
    LOGGER.info(
        "Running within-trace discrimination (%d draws)",
        config.analysis.exploratory_bootstrap_samples,
    )
    ...
    fit_predictions_path = config.extraction.output_dir / "probes" / "fit_predictions.csv"
    if fit_predictions_path.exists():
        ...
        shortcut_controls, shortcut_predictions = shortcut_control_analysis(
            ...
            source_traces,
            seed=config.seed,
            c_grid=config.analysis.control_c_values,
        )
        ...
        LOGGER.info(
            "Running trace-equal-weight sensitivity (%d draws)",
            config.analysis.exploratory_bootstrap_samples,
        )
        ...
        train_fraction, validation_fraction = _frozen_split_fractions(config.extraction.output_dir)
    ...
    control_predictions_path = config.extraction.output_dir / "probes" / "control_predictions.csv"
    if control_predictions_path.exists():
        LOGGER.info(
            "Running paired probe-control intervals (%d draws)",
            config.analysis.exploratory_bootstrap_samples,
        )
    ...
    LOGGER.info(
        "Running subgroup intervals (%d draws)",
        config.analysis.exploratory_bootstrap_samples,
    )
    subgroups = subgroup_outcomes(
        ...
        seed=config.seed,
        confidence_level=config.analysis.confidence_level,
        min_traces=config.analysis.subgroup_min_traces,
    )
    ...
    audit_path = write_failure_audit_sample(
        ...
        examples_per_category=config.analysis.audit_examples_per_category,
        seed=config.seed,
    )
    _plot_temporal_diagnostics(trajectory, permutation_draws, permutation_summary, output)
    ...
    LOGGER.info("CPU analysis complete: %s", output)
    return summary
...
def _frozen_split_fractions(artifact_dir: Path) -> tuple[float, float]:
    path = artifact_dir / "experiment_config.yaml"
    if not path.exists():
        raise FileNotFoundError(f"Missing frozen frozen analysis configuration: {path}")
...
def causal_sensitivity_analysis(
    ...
    *,
    ...
    path = artifact_dir / "interventions" / "individual.csv"
...
def _load_predictions(artifact_dir: Path) -> pd.DataFrame:
    path = artifact_dir / "probes" / "test_predictions.csv"
    frame = pd.read_csv(path)
    required = {
        "trace_id",
        "source",
    ...
    if frame["layer"].nunique() != 1:
        raise ValueError("CPU analysis expects predictions from exactly one frozen layer")
    if frame.duplicated(["trace_id", "step_index"]).any():
...
def _load_threshold(artifact_dir: Path, predictions: pd.DataFrame) -> float:
    artifact = np.load(artifact_dir / "probes" / "directions.npz")
...
def _build_summary(
    *,
    ...
    verdict = json.loads(
        (config.extraction.output_dir / "interventions" / "behavioral_verdict.json").read_text()
    )
    summary = {
        "status": "complete",
        "analysis_type": "cpu_analysis_on_frozen_outputs",
        ...
        "output_dir": str(config.artifacts.analysis_dir),
    }
...
This analysis reuses the frozen test predictions and intervention tables. It does not load the
language model, fit a new hidden-state probe, or inspect activation tensors. The tests are
